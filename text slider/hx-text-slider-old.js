...
var HXTextSlider = function(options) {
  ...
  function makeSlideArray(sd) {
    ...
    if (options.slideScope !== []) {
      for (var i = 0; i < options.slideScope.length; i++) {
        ...
        if (!scopeInSlides) {
          console.log(
            options.slideScope[i] +
              ' is in scope but not in the current slide list.'
          );
        }
      }
    } else {
  ...
  function lookupSlide(slideName) {
    ...
    console.log(
      "Cannot find slide whose id matches the link's target: " + slideName
    );
  }
  ...
  function getSlideHTML(slide) {
    ...
    slideHTML +=
      ...
      '"/>';
    slideHTML +=
      '<h3 class="hx-slide-title" style="border-bottom: 2px solid ' +
    ...
    for (var j = 0; j < slide.folds.length; j++) {
      if (slide.folds[j].header !== '') {
        slideHTML +=
          '<h4 class="hx-togglenext" tabindex="0" ' +
          'aria-expanded="false" aria-controls="hx-folded-' +
          j +
          '">';
        slideHTML += '<span class="fa fa-caret-right"></span> ';
        slideHTML += slide.folds[j].header;
        slideHTML +=
          ' <span class="sr hx-expandnote">Click to expand</span></h4>';
        slideHTML +=
          '<div id="hx-folded-' +
          j +
          '" aria-hidden="true">' +
    ...
    function prevNextHTML(targetList) {
      ...
      targetList.forEach(function(e) {
        ...
        var outOfScope =
          options.slideScope.indexOf(tempslide.id) === -1 &&
          options.slideScope.length > 0;
        html +=
          '<div class="hx-prevnext-icons ' +
  ...
  function getOverviewHTML(slide) {
    var staticFolder = getAssetURL(window.location.href, 'complete');
    var overview =
      '<div class="hxslide-overview-bigbox hxslide-overview-master" ';
    ...
    overview +=
      '<div class="hxslide-overview-leftbox hxslide-overview-container">';
    if (slide.previous) {
      overview += overviewSideHTML(slide.previous.split(','));
    }
    overview += '</div>';
    ...
    // Central icon
    overview +=
      '<div class="hxslide-overview-centerbox hxslide-overview-container">';
    ...
    overview +=
      '<div class="hxslide-overview-rightbox hxslide-overview-container">';
    if (slide.next) {
      overview += overviewSideHTML(slide.next.split(','));
    }
    overview += '</div>';
  ...
  function resizeItems(leftbox, rightbox) {
    ...
    left_width =
      leftbox.width() / Math.ceil(num_left / options.maxIconsTall) - 10;
    right_width =
      rightbox.width() / Math.ceil(num_right / options.maxIconsTall) - 10;
  ...
  function addListeners(slick, slideData) {
    ...
    togglers
      .off('click.hxtog tap.hxtog')
      .on('click.hxtog tap.hxtog', function() {
        ...
        $(this)
          .find('span.hx-expandnote')
          .text('Click to collapse');
        $(this)
          .find('span.hx-collapsenote')
        ...
        $(this)
          .find('span.sr')
          .toggleClass('hx-expandnote hx-collapsenote');
        $(this)
          .next()
          .slideToggle(200);
        if (
    ...
    var waitForData = setInterval(function() {
      ...
      if (typeof slideData[0] !== 'undefined') {
        ...
        if (options.startingSlide) {
          ...
          homeSlideButton.on('click tap', function() {
            goToSlide(options.startingSlide);
          });
        } else {
          addSlide(slick, getSlideHTML(slideData[0]));
          homeSlideButton.on('click tap', function() {
            goToSlide(slideData[0]);
          });
        }
